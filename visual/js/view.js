...
var View = {
    ...
    setEndPos1: function(gridX, gridY) {
        var node, i, endNodes = this.endNodes;
        if (!endNodes) {
            endNodes = this.endNodes = new Array(this.numRows);
            for (i = 0; i < this.numRows; ++i) {
                endNodes[i] = [];
            }
        }
        node = endNodes[gridY][gridX];
        if(node) {
    ...
    setWalkableAt: function(gridX, gridY, value) {
        ...
        if (value) {
            ...
            node.animate(this.nodeStyle.blocked, 100);
            this.zoomNode(node);
        }
    },
